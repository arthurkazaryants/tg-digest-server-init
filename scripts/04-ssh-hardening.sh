...
if [[ "${APPLY_SSH_CHANGES:-false}" != "true" ]]; then
    ...
    log_warn "=========================================="
    log_warn ""
    log_warn "Это КРИТИЧНЫЙ шаг - потом будет сложно подключиться!"
    log_warn ""
    log_warn "Перед применением:"
    log_warn "1. Откройте НОВУЮ SSH сессию"
    log_warn "2. Проверьте новый порт: ssh -p $SSH_PORT $ALLOW_USERS@<сервер>"
    log_warn "3. Только после УСПЕШНОГО входа финализируйте"
    log_warn ""
    log_warn "Чтобы применить SSH изменения, установите:"
